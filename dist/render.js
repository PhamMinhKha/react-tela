...
function assertIsGroup(v) {
    if (!(v && v instanceof Group)) {
        throw new Error('Expected Group');
    }
}
const getText = ({ children: c }) => {
    if (c == null)
        return '';
...
const reconciler = ReactReconciler({
    ...
    prepareUpdate(instance, type, oldProps, newProps, rootContainer, hostContext) {
        ...
        if (type === 'Text') {
            const newValue = getText(newProps);
...
export function render(app, canvas, opts) {
    ...
    reconciler.updateContainer(createElement(ParentContext.Provider, { value: root }, app), container, null /* parentComponent */, null /* callback */);
    return root;
}
